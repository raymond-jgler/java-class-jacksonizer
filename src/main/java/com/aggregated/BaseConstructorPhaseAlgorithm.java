...
public abstract class BaseConstructorPhaseAlgorithm {
    ...
    public static Boolean hasStringLevelDefaultCtor() {
        if (Objects.nonNull(hasStringLevelDefaultCtor) && hasStringLevelDefaultCtor.isPresent()) {
            return hasStringLevelDefaultCtor.get();
        }
        return null;
    }
    ...
    public static String[] makeFieldRegion(Class clazz) {
        String content;
        int newFrom;
        int newTo;
        try {
            content = extractClassContent(clazz, false);
            newFrom = content.indexOf("{", CLASS_KEYWORD_N_NAME_IDX);
            newTo = content.indexOf(";", WRITABLE_CTOR_IDX);
            if (Math.min(newFrom, newTo) == -1) {
                return new String[0];
            }
        } catch (Throwable t) {
        ...
        return content.substring(newFrom, newTo).split(SPACE);
    }
    ...
    public static String[] getFieldRegion(Class clazz) {
        if (Objects.nonNull(TOP_LEVEL_CLAZZ) && TOP_LEVEL_CLAZZ.equals(clazz)) {
            return FIELD_REGION;
        }
        return makeFieldRegion(clazz);
    }
    ...
    public static String extractClassContent(Class given, boolean isOverride) {
        ...
        if (Objects.nonNull(given) && isOverride) {
        ...
        slashedFullName = slashedFullNameBuilder.toString();
