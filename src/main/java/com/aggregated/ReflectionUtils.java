...
public class ReflectionUtils {
  ...
  /**
   * forcefully get the class object from string path name
   *
  ...
  public static boolean isAssignableFrom(Class current, Class<?>... classes) {
    for (Class each : classes) {
      if (current.isAssignableFrom(each)) {
        return true;
      }
    }
    return false;
  ...
  public static boolean isMutator(Method method) {
    return (method.getName().startsWith("set") || method.getName().startsWith("add"))
            && method.getParameterCount() >= 1;
  }

  /**
   * By default, this will return the top-down hierarchical class list, top parent -> so on if no
   * parent, class itself is returned by default. the first class is the top-level parent.
   *
   * @param clazz
  ...
  public static List<Class> buildTopDownClassesFrom(Class clazz, boolean isPreserveOriginalClass) {
    List<Class> topDownClassHierarchyList = new ArrayList<>();
    rcsBuildHierarchyClassList(clazz, topDownClassHierarchyList);
    if (isPreserveOriginalClass) {
      return topDownClassHierarchyList;
    }
    topDownClassHierarchyList.remove(topDownClassHierarchyList.size() - 1);
    return topDownClassHierarchyList;
  }

  /**
   * A head-based recursive method to find the top parent class, down to the passed in class.
   *
   * @param current
   * @return a list of hierarchical class list.
   */
  private static void rcsBuildHierarchyClassList(Class current, List<Class> listAtomicReference) {
    if (Objects.isNull(current) || Object.class.equals(current.getSuperclass())) {
      if (Objects.nonNull(current)) {
        listAtomicReference.add(current);
      }
      return;
    }
  ...
  public static boolean isForbidden(Class CLAZZ, RawClientRuleInput rawInput) {
    ...
    /** Don't skip if there are inner classes */
    ...
    for (String eachSkipped : skippedClasses) {
      if (StringUtils.containsAny(className, eachSkipped)
              ...
              || className.equalsIgnoreCase(eachSkipped)
  ...
  public static boolean hardCodeIsJackson(Class CLAZZ) {
    if (CLAZZ.isAnnotationPresent(JsonDeserialize.class)
            ...
            || CLAZZ.getSimpleName().equalsIgnoreCase("Builder")) {
      return true;
    }
    return false;
  }
  ...
  private static Boolean isFieldUnassigned(String fieldName) {
    ...
    for (int i = 0, n = FIELD_REGION.length; i < n; i++) {
      ...
      if (running.contains("/**") || running.contains("//") || running.contains("/***")) {
        // skip cmt block
        //        for (i += 1; i < n && (!searchRange[i].contains("//") &&
        // !searchRange[i].contains("*/")); i++) {}
        for (i += 1;
             i < n
                     && !StringUtils.containsAny(FIELD_REGION[i], "*/", "**/", "***/", "//", "\n", "\r");
             i++) {}
        i++;
        continue;
      }
      if (running.contains(fieldName)) {
        if (false
                == StringUtils.resolveReplaces(running, "\r", "", "\n", "", " ", "")
                .equals(fieldName)) {
          continue;
        }
        if (running.contains(";")) {
        ...
        for (; i < n; i++) { // ok we'll take it from here then bail.
          ...
          if (running.contains("=")) {
            for (; i < n; i++) {
              running = FIELD_REGION[i];
              if (running.contains("null")
                      || running.contains("false")
                      || running.contains("true")) {
                return true;
              }
              if (StringUtils.containsAny(FIELD_REGION[i], "\r", "\n", ";")) {
                break;
  ...
  public static DecorationLocalField createDecoLocalFieldFrom(Field field) {
    return DecorationLocalField.createFrom(
            field.getName(),
  ...
  public static boolean isSerializableField(Field field) {
    ...
    if ((Modifier.isFinal(modVal) && Modifier.isStatic(modVal))
            ...
            || (Modifier.isFinal(modVal) && !evalBoolean(isFieldUnassigned(field.getName())))) {
      return false;
    }
  ...
  public static List<DecorationLocalField> merge(
          List<DecorationLocalField> from, List<DecorationLocalField> with) {
