...
public class AnnotatableConstructorDecorator extends BaseDecorationAlgorithm {
    ...
    public AnnotatableConstructorDecorator(
    ...
    protected void executePhases(PhaseChainedResult previousInput) {
        ...
        for (BaseConstructorPhaseAlgorithm phase : algorithmPhases) {
            ...
            if (runningPhaseResultOutput.failVerify()) {
                LOG.info(
                        "Class is skipped at phase = "
                                + runningPhaseResultOutput
                                + "need by-hand verifications !");
                break;
            }
        }
    }
    ...
    public static void enqueueWith(Class clz) {
        if (Objects.isNull(clz)
                || lineUps.contains(clz)
                || visited.contains(cleanseClassPath(clz.getName()))
    ...
    /**
     * By default, this will use BFS to explore and process all domain-related superFields. if flag is
     * set to true then BFS to explore all imports as well - CAUTION : IT WILL PROCESS MANY CLASSES.
     *
     * @return
     */
    @Override
    public ExecutionResult execute() {
        ...
        /**
         * Annotation's insertion. BFS to process class superFields, and inner classes , and superFields
         * in them..
         */
        ...
        while (!lineUps.isEmpty()) {
            try {
                ...
                if (rawInput.getWithClassHierarchy()
                        && !visited.contains(cleanseClassPath(popped.getName()))) {
                    /**
                     * PHASE 1 Evaluate constructor if needs parent-merging. This phase will only write a new
                     * ctor if need be.
                     */
                    List<Class> hierarchicalTopDownClasses =
                            ReflectionUtils.buildTopDownClassesFrom(popped, Boolean.TRUE);
                    for (int i = 0, n = hierarchicalTopDownClasses.size(); i < n; i++) {
                        Class curr = hierarchicalTopDownClasses.get(i);
                        final String classPathName = curr.getName();
                        ...
                        if (i > 0) {
                            ...
                            superFields =
                                    BaseConstructorPhaseAlgorithm.getFieldsByClassName(previouslyProcessed.getName());
                            BaseConstructorPhaseAlgorithm.addFieldListToMap(
                                    classPathName, superFields, Boolean.FALSE);
                            BaseConstructorPhaseAlgorithm.addChildren(
                                    previouslyProcessed.getName(), classPathName, Boolean.TRUE);
                        }
                        enqueueWith(curr);
                ...
                /** ONLY IF REQUIRED Collect all domain-based imported classes here enqueue then process. */
                if (rawInput.getBfsImports()) {
                    try {
                        String extractedContent =
                                BaseConstructorPhaseAlgorithm.extractClassContent(popped, false);
                ...
                if (rawInput.getBfsFields()) {
                    for (Field field : popped.getDeclaredFields()) {
                        ...
                        if (field.getType().isPrimitive()
                                ...
                                || visited.contains(fieldTypeName)) {
                            continue;
                        }
                        /** eval generic inside */
     ...
     * @return
     */
    public static Queue<String> divideAndConquerBy(
            String[] splitted, Queue<String> running, char by) {
        for (String each : splitted) {
            if (each.contains(String.valueOf(by))) {
                return divideAndConquerBy(each.split(","), running, by);
            }
            if (StringUtils.containsAny(each, "java.util", "java.lang")
                    || running.contains(StringUtils.stripDoubleEndedNonAlphaNumeric(each))) {
                continue;
            }
            for (String inner : splitted) {
                if (!inner.contains(".")
                        || StringUtils.containsAny(inner, "java.util", "java.lang")
                        || running.contains(inner)) {
                    continue;
                }
    ...
    public static String cleanseClassPath(String raw) {
        String resolved =
                StringUtils.stripUntilClassPath(
                        StringUtils.stripDoubleEndedNonAlphaNumeric(
                                StringUtils.resolveReplaces(
                                        raw, "class ", "", ">", "", "<", "", "[", "", "]", "", "[]", "")),
                        '.',
                        '$',
                        '_');
    ...
    private void enqueueWithImportedDomainClasses(String content) {
        ...
        for (String each : splitted) {
            ...
            try {
                ...
                if (StringUtils.containsAny(importLine, "Builder", "Util")
                        || ReflectionUtils.isForbidden(clazz, rawInput)
                        || ReflectionUtils.hardCodeIsJackson(clazz)) {
                    continue;
                }
                lineUps.offer(clazz);
    ...
    /**
     * recursively add constructors to inner classes
     *
     * @param inners
     */
